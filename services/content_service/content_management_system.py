...
class FileService:
    ...
    async def _create_processing_jobs(self, file: File):
        """Создание заданий на обработку файла"""
        # Для изображений создаем миниатюры
        if file.file_type == FileType.IMAGE:
            await self.create_thumbnail(file.id, "small")
            await self.create_thumbnail(file.id, "medium")
        # Для видео создаем миниатюру и возможно перекодировку
            ...
            await self._create_video_transcoding_job(file.id, "mp4", "web_optimized")
        # Для документов создаем превью
        elif file.file_type == FileType.DOCUMENT:
            await self._create_document_preview_job(file.id)

    async def _create_video_transcoding_job(self, file_id: str, target_format: str, 
                                          quality: str) -> Optional[str]:
        """Создание задания на перекодировку видео"""
        job_input_params = {
            "file_id": file_id,
            "target_format": target_format,
            "quality": quality
        ...
        return await self._create_processing_job(
            file_id, 0, FileProcessingType.VIDEO_TRANSCODING, job_input_params
        )

    async def _create_document_preview_job(self, file_id: str) -> Optional[str]:
        """Создание задания на создание превью документа"""
        job_input_params = {
            "file_id": file_id
        }

        return await self._create_processing_job(
            file_id, 0, FileProcessingType.DOCUMENT_PREVIEW, job_input_params
        )
