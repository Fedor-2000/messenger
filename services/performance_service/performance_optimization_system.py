...
# File: services/performance_service/performance_optimization_system.py
    ...
    async def _create_optimal_indexes(self, query: str):
        ...
        try:
            ...
            if where_match:
                ...
                # Ищем все поля, используемые в условиях
                ...
                fields_to_index.extend(matches)

                # Также ищем поля в JOIN-ах
                join_matches = re.findall(r'JOIN\s+\w+\s+ON\s+\w+\.(\w+)|ON\s+\w+\.(\w+)\s*=',
                                         where_clause, re.IGNORECASE)
                for match in join_matches:
                    fields_to_index.extend([m for m in match if m])  # добавляем непустые совпадения

            # Удаляем дубликаты и пустые значения
            fields_to_index = list(set(filter(None, fields_to_index)))

            if fields_to_index:
                # Создаем составные индексы для часто используемых комбинаций полей
                for field in fields_to_index:
                    index_name = f"perf_idx_{table_name}_{field}"
                    ...
                    try:
                        async with db_pool.acquire() as conn:
                            ...
                            if not existing_idx:
                                # Создаем индекс
                                await conn.execute(f"CREATE INDEX CONCURRENTLY {index_name} ON {table_name} ({field})")

                                # Логируем создание индекса
                                await conn.execute(
                                    """
                                    INSERT INTO optimization_logs (type, name, table_name, fields, created_at)
                                    VALUES ($1, $2, $3, $4, $5)
                                    """, "index", index_name, table_name, [field], datetime.utcnow()
                                )

                                logger.info(f"Created performance index {index_name} on {table_name}.{field}")
                            else:
