...
# File: services/theme_service/theme_management.py
...
class ThemeManagementService:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                ON CONFLICT (id) DO UPDATE SET
                    name = $2, type = $3, colors = $4, font_family = $5,
                    font_size = $6, spacing = $7, corner_radius = $8,
                    shadow_intensity = $9, animation_speed = $10, is_public = $12,
                    updated_at = $15
                """,
    ...
    async def _cache_user_theme_preference(self, preference: UserThemePreference):
        ...
        await redis_client.setex(
            f"user_theme_pref:{preference.user_id}",
            1800,  # 30 минут
    ...
    async def update_theme(self, theme_id: str, user_id: int, 
                          ...
                          animation_speed: Optional[str] = None) -> bool:
        """Обновление темы"""
        theme = await self.get_theme(theme_id)
        if not theme:
            return False

        # Проверяем права на обновление (только создатель или администратор)
        if theme.creator_id != user_id and not await self._is_admin(user_id):
            return False
        ...
        if animation_speed is not None:
            theme.animation_speed = animation_speed
        ...
        # Создаем запись активности
        await self._log_activity("theme_updated", {
            "theme_id": theme_id,
            "updated_fields": [field for field in ['name', 'colors', 'font_family', 'font_size', 'spacing', 
                                                'corner_radius', 'shadow_intensity', 'animation_speed'] 
    ...
    async def _update_theme_in_db(self, theme: Theme):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                """
                UPDATE themes SET
                    name = $2, colors = $3, font_family = $4, font_size = $5,
                    spacing = $6, corner_radius = $7, shadow_intensity = $8,
                    animation_speed = $9, updated_at = $10
                WHERE id = $1
                """,
                theme.id, theme.name, json.dumps(theme.colors), theme.font_family,
                theme.font_size, theme.spacing, theme.corner_radius,
                theme.shadow_intensity, theme.animation_speed, theme.updated_at
            )

    async def delete_theme(self, theme_id: str, user_id: int) -> bool:
        """Удаление темы"""
        theme = await self.get_theme(theme_id)
        if not theme or theme.is_default:
            return False
        ...
        # Проверяем, используется ли тема кем-то
        users_using_theme = await self._get_users_using_theme(theme_id)
        if users_using_theme:
            # Не позволяем удалить тему, если она используется
            # Вместо этого помечаем как не публичную
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("DELETE FROM themes WHERE id = $1", theme_id)

        # Удаляем из кэша
        await redis_client.delete(f"theme:{theme_id}")

        # Создаем запись активности
        await self._log_activity("theme_deleted", {
            "theme_id": theme_id,
    ...
    async def _get_users_using_theme(self, theme_id: str) -> List[int]:
        """Получение пользователей, использующих тему"""
        async with db_pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT user_id FROM user_theme_preferences WHERE theme_id = $1",
                theme_id
            )
    ...
    async def get_theme_statistics(self) -> Dict[str, Any]:
        ...
        async with db_pool.acquire() as conn:
            # Общая статистика
            total_themes = await conn.fetchval("SELECT COUNT(*) FROM themes")
            public_themes = await conn.fetchval("SELECT COUNT(*) FROM themes WHERE is_public = true")
            user_created_themes = await conn.fetchval("SELECT COUNT(*) FROM themes WHERE is_default = false")

            # Статистика по типам тем
            type_stats = await conn.fetch(
                "SELECT type, COUNT(*) as count FROM themes GROUP BY type"
            )

            # Статистика использования
            usage_stats = await conn.fetch(
                """
                SELECT tp.name, tp.type, COUNT(utp.user_id) as usage_count
                FROM themes tp
                LEFT JOIN user_theme_preferences utp ON tp.id = utp.theme_id
                GROUP BY tp.id, tp.name, tp.type
        ...
        return {
            ...
            'public_themes': public_themes or 0,
            'user_created_themes': user_created_themes or 0,
            'themes_by_type': {row['type']: row['count'] for row in type_stats},
            'most_used_themes': [
                {
                    'name': row['name'],
                    'type': row['type'],
    ...
    async def share_theme(self, theme_id: str, user_id: int, 
        ...
        # Проверяем права на предоставление доступа к теме
        if theme.creator_id != user_id and not theme.is_public:
            return False

        # Если тема приватная, делаем ее публичной
