...
# File: services/ux_service/user_experience_enhancement_system.py
...
class UXEnhancementService:
    ...
    async def _update_behavior_profile_in_db(self, profile: UserBehaviorProfile):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                profile.peak_usage_times, profile.preferred_features,
                profile.last_interaction, profile.engagement_score,
                profile.churn_risk_score, profile.updated_at
            )
    ...
    async def submit_user_feedback(self, user_id: int, feedback_type: FeedbackType,
                                 ...
                                 rating: Optional[int] = None) -> Optional[str]:
    ...
    async def get_user_satisfaction_insights(self, user_id: int) -> Dict[str, Any]:
        """Получение инсайтов о удовлетворенности пользователя"""
        # Получаем среднюю оценку удовлетворенности
        avg_satisfaction = await self._get_average_user_satisfaction(user_id)
        
        # Получаем последние отзывы
        recent_feedback = await self._get_user_feedback(user_id)
        
        # Анализируем тенденции
        satisfaction_trends = await self._analyze_satisfaction_trends(user_id)
        ...
        insights = {
            ...
            'satisfaction_improvement_areas': await self._identify_satisfaction_improvement_areas(user_id),
            'timestamp': datetime.utcnow().isoformat()
        }
        
        return insights

    async def _get_average_user_satisfaction(self, user_id: int) -> float:
        """Получение средней оценки удовлетворенности пользователя"""
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                SELECT AVG(satisfaction_score) as avg_satisfaction
                FROM user_interactions
                WHERE user_id = $1 AND satisfaction_score IS NOT NULL
                """,
                user_id
            )
        
        return float(row['avg_satisfaction']) if row and row['avg_satisfaction'] else 3.0  # По умолчанию нейтральная оценка

    async def _analyze_satisfaction_trends(self, user_id: int) -> Dict[str, Any]:
        """Анализ тенденций удовлетворенности"""
        # Получаем удовлетворенность за последние 30 дней
        thirty_days_ago = datetime.utcnow() - timedelta(days=30)
        
        async with db_pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT DATE_TRUNC('day', timestamp) as day, AVG(satisfaction_score) as avg_satisfaction
                FROM user_interactions
                WHERE user_id = $1 AND timestamp >= $2 AND satisfaction_score IS NOT NULL
        ...
        for row in rows:
            ...
            daily_satisfaction[day] = float(row['avg_satisfaction'])
        
        # Рассчитываем тренд
        satisfaction_values = list(daily_satisfaction.values())
        if len(satisfaction_values) >= 2:
            trend_direction = "increasing" if satisfaction_values[-1] > satisfaction_values[0] else "decreasing"
            trend_strength = abs(satisfaction_values[-1] - satisfaction_values[0])
        else:
        ...
        return {
            ...
            'latest_satisfaction': satisfaction_values[-1] if satisfaction_values else 3.0
        }

    def _calculate_positive_feedback_ratio(self, feedback_list: List[UserFeedback]) -> float:
        """Расчет отношения положительных отзывов"""
        if not feedback_list:
            return 0.0
        
        positive_count = sum(1 for fb in feedback_list if fb.rating and fb.rating >= 4)
        return positive_count / len(feedback_list)

    async def _identify_satisfaction_improvement_areas(self, user_id: int) -> List[Dict[str, str]]:
        """Идентификация областей для улучшения удовлетворенности"""
        areas = []
        
        # Проверяем, есть ли негативные отзывы
        negative_feedback = await self._get_negative_feedback(user_id)
        
        for feedback in negative_feedback:
            if 'performance' in feedback.description.lower() or 'slow' in feedback.description.lower():
                areas.append({
                    'area': 'performance',
                    'issue': 'Slow response times or performance issues',
                    'suggested_solution': 'Optimize system performance and implement caching'
                })
            elif 'interface' in feedback.description.lower() or 'navigation' in feedback.description.lower():
                areas.append({
                    'area': 'interface',
                    'issue': 'Interface or navigation difficulties',
                    'suggested_solution': 'Redesign interface for better usability'
                })
            elif 'feature' in feedback.description.lower():
                areas.append({
                    'area': 'functionality',
                    'issue': 'Missing or problematic features',
                    'suggested_solution': 'Implement requested features or fix existing ones'
                })
        
        return areas

    async def _get_negative_feedback(self, user_id: int) -> List[UserFeedback]:
        """Получение негативных отзывов пользователя"""
        async with db_pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT id, user_id, feedback_type, title, description, rating,
                       is_resolved, resolved_at, resolution_notes, created_at, updated_at
                FROM user_feedback
                WHERE user_id = $1 AND (rating IS NULL OR rating < 3)
                ORDER BY created_at DESC
                LIMIT 10
                """,
                user_id
